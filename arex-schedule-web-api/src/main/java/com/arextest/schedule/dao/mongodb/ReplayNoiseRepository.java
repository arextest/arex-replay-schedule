...
public class ReplayNoiseRepository implements RepositoryField {
    ...
    public boolean saveList(List<ReplayNoiseDto> replayNoiseDtoList) {
        ...
        List<ReplayNoiseCollection> collect =
            replayNoiseDtoList.stream().map(ReplayNoiseConverter.INSTANCE::daoFromDto).collect(Collectors.toList());
        ...
        try {
            ...
            for (ReplayNoiseCollection replayNoiseCollection : collect) {
                Query query = Query
                    .query(Criteria.where(ReplayNoiseCollection.FIELD_PLAN_ID).is(replayNoiseCollection.getPlanId())
                        .and(ReplayNoiseCollection.FIELD_PLAN_ITEM_ID).is(replayNoiseCollection.getPlanItemId())
                        .and(ReplayNoiseCollection.FIELD_CATEGORY_NAME).is(replayNoiseCollection.getCategoryName())
                        .and(ReplayNoiseCollection.FIELD_OPERATION_NAME).is(replayNoiseCollection.getOperationName()));
                ...
                update.setOnInsert(ReplayNoiseCollection.FIELD_PLAN_ITEM_ID, replayNoiseCollection.getPlanItemId());
                update.setOnInsert(ReplayNoiseCollection.FIELD_CATEGORY_NAME, replayNoiseCollection.getCategoryName());
                update.setOnInsert(ReplayNoiseCollection.FIELD_OPERATION_ID, replayNoiseCollection.getOperationId());
    ...
    public boolean removeReplayNoise(List<String> planItemIds) {
        ...
        Query query = Query.query(Criteria.where(ReplayNoiseCollection.FIELD_PLAN_ITEM_ID).in(planItemIds));
    ...
    public List<ReplayNoiseDto> queryReplayNoise(@NotBlank String planId, String planItemId) {
        ...
        List<ReplayNoiseCollection> replayNoiseCollections = mongoTemplate.find(query, ReplayNoiseCollection.class);
        return replayNoiseCollections.stream().map(ReplayNoiseConverter.INSTANCE::dtoFromDao)
            .collect(Collectors.toList());
    }

    private void appendUpdate(Update update, Map<String, ReplayNoiseCollection.ReplayNoiseItemDao> needUpdateContent,
        String updateKey) {
        if (MapUtils.isNotEmpty(needUpdateContent)) {
            for (Map.Entry<String, ReplayNoiseCollection.ReplayNoiseItemDao> entry : needUpdateContent.entrySet()) {
                ...
                update.set(
                    MongoHelper.appendDot(updateKey, path, ReplayNoiseCollection.ReplayNoiseItemDao.FIELD_NODE_PATH),
                ...
                update.set(
                    MongoHelper.appendDot(updateKey, path, ReplayNoiseCollection.ReplayNoiseItemDao.FIELD_LOG_INDEXES),
                    replayNoiseItemDao.getLogIndexes());
                update.inc(
                    MongoHelper.appendDot(updateKey, path, ReplayNoiseCollection.ReplayNoiseItemDao.FIELD_PATH_COUNT),
                    replayNoiseItemDao.getPathCount());
                update.inc(
                    MongoHelper.appendDot(updateKey, path, ReplayNoiseCollection.ReplayNoiseItemDao.FIELD_CASE_COUNT),
