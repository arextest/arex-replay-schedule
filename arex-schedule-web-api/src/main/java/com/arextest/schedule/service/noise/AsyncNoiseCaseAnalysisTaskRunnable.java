...
public class AsyncNoiseCaseAnalysisTaskRunnable extends AbstractTracedRunnable {
    ...
    protected void doWithTracedRunning() {
        ...
        LOGGER.info("start to analysis noise case, planItemId: {}, contextName: {}", planItemId, contextName);
        try {
            ...
            for (ReplayActionCaseItem caseItem : cases) {
                ...
                if (caseItem.getSourceResultId() == null && caseItem.getTargetResultId() == null) {
                    CompareResult compareResult =
                        CompareSDK.fromException(caseItem.requestMessage(), null, caseItem.getSendErrorMessage());
            ...
            Map<CompareResultGroupIdentify, List<ReplayCompareResult>> compareResultOfCategory = compareResults.stream()
                ...
                .collect(
                    Collectors.groupingBy(item -> new CompareResultGroupIdentify(item.getCategoryName(), item.getOperationId(),item.getOperationName())));
            ...
            replayPlanActionRepository.updateNoiseOfContextFinished(planItemId, contextName, cases.size());
        } catch (RuntimeException exception) {
            LOGGER.error("analysis noise case failed, planItemId: {}, contextName: {}", planItemId, contextName,
                exception);
        }
        LOGGER.info("finish to analysis noise case, planItemId: {}, contextName: {}", planItemId, contextName);
    ...
    private InferredReplayNoiseDto analysisNoiseFromCompareResult(List<ReplayCompareResult> compareResults) {
        ...
        for (ReplayCompareResult compareResult : compareResults) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (!ifFindArray) {
                    this.getAnalysisOfSingleCompareResult(mayIgnoreNodesOfSingle, upperArrayNodePath, compareResult,
                        log, i);
                } else {
                    this.getAnalysisOfSingleCompareResult(mayDisorderArrayOfSingle, upperArrayNodePath, compareResult,
                        log, i);
                }
            }
    ...
    private void getAnalysisOfSingleCompareResult(Map<List<NodeEntity>, ReplayNoiseItemDto> singleAggContent,
    ...
    private void toNoiseCompareResult(List<ReplayCompareResult> compareResults) {
        for (ReplayCompareResult replayCompareResult : compareResults) {
            ...
            replayCompareResult.setPlanItemId(replayCompareResult.getPlanItemId() + CommonConstant.NOISE_HANDLER);
            replayCompareResult.setRecordId(replayCompareResult.getRecordId() + CommonConstant.NOISE_HANDLER);
            replayCompareResult.setReplayId(replayCompareResult.getReplayId() + CommonConstant.NOISE_HANDLER);
        }
    }
