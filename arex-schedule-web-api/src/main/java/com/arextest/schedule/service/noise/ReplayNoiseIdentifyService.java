...
public class ReplayNoiseIdentifyService implements ReplayNoiseIdentify {
    ...
    public ReplayNoiseIdentifyService(CompareConfigService compareConfigService, ProgressTracer progressTracer,
    ...
    @Override
    public void noiseIdentify(List<ReplayActionCaseItem> allCasesOfContext, PlanExecutionContext<?> executionContext) {
        ...
        CreateReplayNoiseSendTaskRequest request = new CreateReplayNoiseSendTaskRequest(replayActionCaseItems, limiter);
    ...
    public void rerunNoiseAnalysisRecovery(List<ReplayActionItem> actionItems) {
        ...
        for (ReplayActionItem actionItem : actionItems) {
            ...
            Set<String> failedCaseIdentifiers =
                caseItemList.stream().map(ReplayActionCaseItem::getContextIdentifier).collect(Collectors.toSet());
        ...
        List<String> failedActions = actionItems.stream().map(ReplayActionItem::getId).collect(Collectors.toList());
    ...
    public QueryNoiseResponseType queryNoise(String planId, String planItemId) {
        ...
        List<ReplayNoiseDto> replayNoiseDtoList = replayNoiseRepository.queryReplayNoise(planId, planItemId);
        ...
        for (Map.Entry<String, List<ReplayNoiseDto>> interfaceReplayNoiseEntry : interfaceReplayNoiseMap.entrySet()) {
            ...
            for (ReplayNoiseDto replayNoiseDto : interfaceReplayNoise) {
                ...
                if (MapUtils.isNotEmpty(mayDisorderItems)) {
                    ...
                    List<QueryNoiseResponseType.NoiseItem> disorderedArrayNoiseItems = new ArrayList<>(values.size());
                    for (ReplayNoiseItemDto itemDtoEntry : values) {
            ...
            if (CollectionUtils.isNotEmpty(randomNoiseList) || CollectionUtils.isNotEmpty(disorderedArrayNoise)) {
                appNoiseList.add(
                    new QueryNoiseResponseType.InterfaceNoiseItem(operationId, randomNoiseList, disorderedArrayNoise));
            }
        }
    ...
    private void analysisNoise(ActionItemForNoiseIdentify actionItemForNoiseIdentify) {
        ...
        asyncNoiseCaseAnalysisTaskRunnable.setReplayCompareResultRepository(replayCompareResultRepository);
    ...
    private void doReplayNoiseSendTasks(CreateReplayNoiseSendTaskRequest request) {
        ...
        try {
            boolean await = countDownLatch.await(CommonConstant.GROUP_SENT_WAIT_TIMEOUT_SECONDS, TimeUnit.SECONDS);
            if (!await) {
