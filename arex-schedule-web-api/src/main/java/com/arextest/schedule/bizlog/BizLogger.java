...
public class BizLogger {
    ...
    public static void recordPlanDone(ReplayPlan plan) {
        BizLog log =
            BizLog.info().logType(BizLogContent.PLAN_DONE.getType()).message(BizLogContent.PLAN_DONE.format()).build();
    ...
    public static void recordPlanException(ReplayPlan plan, Throwable t) {
        BizLog log = BizLog.error().logType(BizLogContent.PLAN_FATAL_ERROR.getType())
            .message(BizLogContent.PLAN_FATAL_ERROR.format()).exception(BizLogContent.throwableToString(t)).build();
    ...
    public static void recordActionItemCaseSaved(ReplayActionItem action, int saveCount, long elapsed) {
        BizLog log = BizLog.info().logType(BizLogContent.ACTION_ITEM_CASE_SAVED.getType())
            .message(BizLogContent.ACTION_ITEM_CASE_SAVED.format(action.getOperationName(), saveCount, elapsed))
            .build();
    ...
    public static void recordActionItemCaseCount(ReplayActionItem action) {
        BizLog log = BizLog.info().logType(BizLogContent.ACTION_ITEM_INIT_TOTAL_COUNT.getType())
            .message(BizLogContent.ACTION_ITEM_INIT_TOTAL_COUNT.format(action.getOperationName(), action.getId(),
    ...
    public static void recordActionStatusChange(ReplayActionItem action, String targetStatus, String reason) {
        BizLog log = BizLog.info().logType(BizLogContent.ACTION_ITEM_STATUS_CHANGED.getType())
            .message(BizLogContent.ACTION_ITEM_STATUS_CHANGED.format(action.getOperationName(), action.getId(),
                targetStatus, reason))
            .build();
    ...
    public static void recordContextBuilt(ReplayPlan plan, long elapsed) {
        BizLog log = BizLog.info().logType(BizLogContent.PLAN_CONTEXT_BUILT.getType())
            .message(BizLogContent.PLAN_CONTEXT_BUILT
                .format(Optional.ofNullable(plan.getExecutionContexts()).map(Collection::size).orElse(0), elapsed))
            .build();
    ...
    public static void recordContextBeforeRun(PlanExecutionContext<?> context, long elapsed) {
        BizLog log = BizLog.info().logType(BizLogContent.CONTEXT_START.getType())
            .message(
                BizLogContent.CONTEXT_START.format(context.getContextName(), context.getActionType().name(), elapsed))
            .build();
    ...
    public static void recordContextSkipped(PlanExecutionContext<?> context, ReplayActionItem actionItem,
        long skipCount) {
    ...
    public static void recordCaseForNoiseSendStart(@NotNull PlanExecutionContext<?> context, int sentCount) {
        BizLog log = BizLog.info().logType(BizLogContent.NOISE_IDENTIFY_CASE_SEND_START.getType())
                .message(BizLogContent.NOISE_IDENTIFY_CASE_SEND_START.format(context.getContextName(), sentCount)).build();
        log.postProcessAndEnqueue(context);
    }

    public static void recordCaseForNoiseSendFinish(@NotNull PlanExecutionContext<?> context, int sentCount, long elapsedMills) {
        BizLog log = BizLog.info().logType(BizLogContent.NOISE_IDENTIFY_CASE_SEND_FINISH.getType())
            .message(BizLogContent.NOISE_IDENTIFY_CASE_SEND_FINISH.format(context.getContextName(), sentCount, elapsedMills)).build();
            log.postProcessAndEnqueue(context);
    }
    // endregion
    ...
    public enum BizLogContent {
        ...
        PLAN_DONE(3, "Plan send job done normally."), PLAN_ASYNC_RUN_START(4, "Plan async task init."),
        ...
        QPS_LIMITER_RESET(102, "Qps limit reset initial status to {0}."),
        ...
        @Deprecated ACTION_ITEM_EXECUTE_CONTEXT(300,
        ...
        @Deprecated ACTION_ITEM_SENT(304, "All cases of Operation: {0} id: {1} sent, total size: {2}"),
        @Deprecated ACTION_ITEM_BATCH_SENT(305, "Batch cases of Operation: {0} id: {1} sent, size: {2}"),
        ...
        NOISE_IDENTIFY_CASE_SEND_FINISH(501, "Context: {0}, {1} case finish sending to identify noise, took {2} ms."),
