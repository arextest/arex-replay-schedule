...
/**
 * Created by Qzmo on 2023/5/15 For test usage only, to simulate multiple version of execution
 * context
 */
@Slf4j
public class MockMultiVersionExecutionImpl implements
    PlanExecutionContextProvider<Map<String, String>> {
  ...
  @Override
  public void onBeforeContextExecution(PlanExecutionContext<Map<String, String>> currentContext,
      ReplayPlan plan) {
    MDCTracer.addExecutionContextNme(currentContext.getContextName());
    currentContext.setActionType(
        new Random().nextInt(10) >= 5 ? ExecutionContextActionType.SKIP_CASE_OF_CONTEXT
            : ExecutionContextActionType.NORMAL);
    LOGGER.info("Start executing context: {}", currentContext);
    // prepare dependencies before sending any cases of this context...
  }

  @Override
  public void onAfterContextExecution(PlanExecutionContext<Map<String, String>> currentContext,
      ReplayPlan plan) {
